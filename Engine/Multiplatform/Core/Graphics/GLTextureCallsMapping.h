...
namespace Neptune
{
	namespace GLTextureCallsMapping
	{
		...
		void GLTexSubImage(const Texture::MetaData& _metaData, u8 _mipmapLevel, const void** _data);
		
		// Cube-map specifics
		void GLTexStorageCubeMap(const Texture::MetaData& _metaData);
		void GLTexStorageCubeMapArray(const Texture::MetaData& _metaData);
		void GLTexSubImageCubeMap(const Texture::MetaData& _metaData, u8 _mipmapLevel, const void* _data);
		void GLTexSubImageCubeMapArray(const Texture::MetaData& _metaData, u8 _mipmapLevel, const void** _data);
	}
}
