...
namespace Neptune
{
	namespace DisplayDeviceInterface
	{
		...
		struct GraphicalContextSettings
		{
			MULTI_SAMPLE_ANTI_ALLIASING	m_antiAliasing;			/// Enable anti aliasing through multi-sampling. Default is set to MULTI_SAMPLE_ANTI_ALLIASING::NONE.
		...
		/// \pm4 (optional) user settings to customize the graphical context (add multi-sampled anti-aliasing, depth tests using the reversed-z technique....).
		GraphicalContextHandle CreateGraphicalContext(WindowHandle _window, u8 _openGLMinorVersion, u8 _openGLMajorVersion, GraphicalContextSettings _userSettings = GraphicalContextSettings());
